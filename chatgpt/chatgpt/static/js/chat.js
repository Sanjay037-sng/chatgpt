class ChatApp {
    ...
    init() {
        ...
        this.uploadButton.addEventListener('click', (e) => this.handleFileUpload(e));
    ...
    async handleFileSelect(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showUploadStatus(`✅ Document "${data.title}" uploaded successfully!`);
                ...
                const message = `I've uploaded a document: ${data.title}. You can now ask me to analyze, summarize, or answer questions about it.`;
                this.messageInput.value = message;
                this.handleInputResize();
            } else {
                const errorData = await response.json();
                this.showUploadError(`❌ Upload failed: ${errorData.error}`);
            }
        } catch (error) {
    ...
    displayDocuments(documents) {
        ...
        if (documents.length === 0) {
            const noDocumentsDiv = document.createElement('div');
            noDocumentsDiv.className = 'no-documents';
            noDocumentsDiv.style.cssText = `
                padding: 16px;
                text-align: center;
                color: #9ca3af;
                font-size: 14px;
                font-style: italic;
            `;
            noDocumentsDiv.textContent = 'No documents uploaded yet.';
            this.documentsList.appendChild(noDocumentsDiv);
            return;
        }
    ...
    createDocumentElement(document) {
        ...
        div.innerHTML = `
            ...
            <div class="document-actions">
                <button class="document-action" onclick="event.stopPropagation(); chatApp.deleteDocument(${document.id})" title="Delete">
                    🗑️
                </button>
            </div>
        `;
        
        div.addEventListener('click', () => this.loadDocument(document.id));
    ...
    async loadDocument(documentId) {
        try {
            ...
            const message = `Please analyze this document: ${document.title}. Here's what I want to know about it:`;
            this.messageInput.value = message;
            this.handleInputResize();
            this.messageInput.focus();
    ...
    async deleteDocument(documentId) {
        ...
        try {
            ...
            if (response.ok) {
                // Remove from UI
                const docElement = document.querySelector(`[onclick*="${documentId}"]`);
                if (docElement) {
                    docElement.remove();
                }
